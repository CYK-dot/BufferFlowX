 ...
 */
#define BFX_QFIFO_INIT(pFifo, pBuf, uiSize) do { \
    (pFifo)->buf = (pBuf);                     \
    (pFifo)->size = (uiSize);                  \
    (pFifo)->headReady = 0;                    \
    (pFifo)->tailReady = 0;                    \
 ...
 */
#define BFX_QFIFO_FREE_SIZE(pFifo)                              \
 ...
 */
#define BFX_QFIFO_FREE_NOSPLIT_SIZE(pFifo)                               \
    ((pFifo)->tailReady > (pFifo)->headPend ?                            \
    (pFifo)->tailReady - (pFifo)->headPend - 1 :                         \
 ...
 */
#define BFX_QFIFO_FREE_VARI_SIZE(pFifo)                          \
 ...
 */
#define BFX_QFIFO_RECV_SIZE(pFifo)                          \
 ...
 */
#define BFX_QFIFO_RECV_NOSPLIT_SIZE(pFifo)     \
    ((pFifo)->headReady >= (pFifo)->tailPend ? \
     (pFifo)->headReady - (pFifo)->tailPend :  \
 ...
 */
#define BFX_QFIFO_RECV_VARI_SIZE(pFifo)                      \
 ...
 */
#define BFX_QFIFO_SEND_ACQUIRE_NOSPLIT(pFifo, uiSize, pData, uiAcquiredSize)      \
do {                                                                              \
 ...
 */
#define BFX_QFIFO_SEND_ACQUIRE_SPLIT(pFifo, uiSize, ppData, ppAcquiredSize)         \
do {                                                                                \
    if (BFX_QFIFO_UNLIKELY(((pFifo)->headReady != (pFifo)->headPend) ||             \
 ...
 * @param ppAcquiredSize [OUT] [uint16_t[2]] Pointer to the acquired size array, can be smaller than uiSize.
 */
#define BFX_QFIFO_SEND_ACQUIRE_VARI(pFifo, ppData, ppAcquiredSize)          \
do {                                                                        \
    if (BFX_QFIFO_UNLIKELY((pFifo)->headReady != (pFifo)->headPend)) {      \
            (ppData)[0] = NULL;                                             \
            (ppData)[1] = NULL;                                             \
 ...
 * @param pFifo [IN] [BFX_QFIFO *] Pointer to the FIFO structure.
 */
#define BFX_QFIFO_SEND_COMMIT(pFifo) \
 ...
 */
#define BFX_QFIFO_SEND_COMMIT_VARI(pFifo, uiSize)                       \
do {                                                                    \
    if (BFX_QFIFO_UNLIKELY((pFifo)->headReady == (pFifo)->headPend ||   \
 ...
 * @param pFifo [IN] [BFX_QFIFO *] Pointer to the FIFO structure.
 */
#define BFX_QFIFO_SEND_UNDO(pFifo) \
    (pFifo)->headPend = (pFifo)->headReady
 ...
 * @param uiAcquiredSize [OUT] [uint16_t]  Pointer to the acquired size, can be smaller than uiSize.
 */
#define BFX_QFIFO_RECV_ACQUIRE_NOSPLIT(pFifo, uiSize, pData, uiAcquiredSize)      \
do {                                                                              \
 ...
 * @param ppAcquiredSize [OUT] [uint16_t[2]] Pointer to the acquired size array, can be smaller than uiSize.
 */
#define BFX_QFIFO_RECV_ACQUIRE_SPLIT(pFifo, uiSize, ppData, ppAcquiredSize)         \
do {                                                                                \
    if (BFX_QFIFO_UNLIKELY(((pFifo)->tailReady != (pFifo)->tailPend ||              \
 ...
 * @param ppAcquiredSize [OUT] [uint16_t[2]] Pointer to the acquired size array, can be smaller than uiSize.
 */
#define BFX_QFIFO_RECV_ACQUIRE_VARI(pFifo, ppData, ppAcquiredSize)          \
do {                                                                        \
    if (BFX_QFIFO_UNLIKELY((pFifo)->tailReady != (pFifo)->tailPend)) {      \
            (ppData)[0] = NULL;                                             \
            (ppData)[1] = NULL;                                             \
 ...
 * @param pFifo [IN] [BFX_QFIFO *] Pointer to the FIFO structure.
 */
#define BFX_QFIFO_RECV_COMMIT(pFifo) \
 ...
 */
#define BFX_QFIFO_RECV_COMMIT_VARI(pFifo, size)                         \
do {                                                                    \
    if (BFX_QFIFO_UNLIKELY((pFifo->tailReady == (pFifo)->tailPend) ||   \
 ...
 * @param pFifo [IN] [BFX_QFIFO *] Pointer to the FIFO structure.
 */
#define BFX_QFIFO_RECV_UNDO(pFifo) \
    (pFifo)->tailPend = (pFifo)->tailReady
 ...
 */
#define BFX_QFIFO_PIECE_GET_BYTE(hPiece, i) \
