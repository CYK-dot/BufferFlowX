...
protected:
    ...
    void SetUp() override {
        BFX_QfifoInit(&fifo, buffer, BUFFER_SIZE);
    ...
    void fillFifo(uint16_t dataSize) {
        ...
        for (uint16_t i = 0; i < dataSize; i++) {
            char *data;
            uint16_t acquired;
            BFX_QfifoSendAcquireNoSplit(&fifo, 1, &data, &acquired);
            ASSERT_EQ(acquired, 1);
            ASSERT_NE(data, nullptr);
            BFX_QfifoSendCommit(&fifo);
        }
    }
    void fetchFifo(uint16_t dataSize) {
        ...
        for (uint16_t i = 0; i < dataSize; i++) {
            char *data;
            uint16_t acquired;
            BFX_QfifoRecvAcquireNoSplit(&fifo, 1, &data, &acquired);
            ASSERT_EQ(acquired, 1);
            ASSERT_NE(data, nullptr);
            BFX_QfifoRecvCommit(&fifo);
        }
    }
...
TEST_F(EmFifoTestNoSplit, Initialization) {
    ...
    uint16_t freeSize = BFX_QfifoFreeSize(&fifo);
    ASSERT_EQ(freeSize, BUFFER_SIZE - 1);

    uint16_t recvSize = BFX_QfifoRecvSize(&fifo);
    ASSERT_EQ(recvSize, 0);

    uint16_t freeNoSplitSize = BFX_QfifoFreeNoSplitSize(&fifo);
    ASSERT_EQ(freeNoSplitSize, BUFFER_SIZE - 1);

    uint16_t recvSizeNoSplit = BFX_QfifoRecvNoSplitSize(&fifo);
...
TEST_F(EmFifoTestNoSplit, SendAndReceiveFromStartToMiddle) {
    ...
    BFX_QfifoSendAcquireNoSplit(&fifo, SEND_SIZE, &sendPos, &sendAcquiredSize);
    memcpy(sendPos, strToSend, SEND_SIZE);
    BFX_QfifoSendCommit(&fifo);
    
    BFX_QfifoRecvAcquireNoSplit(&fifo, RECV_SIZE, &recvPos, &recvAcquiredSize);
...
TEST_F(EmFifoTestNoSplit, SendAndReceiveFromMiddleToBoundMiddle) {
    ...
    BFX_QfifoSendAcquireNoSplit(&fifo, SEND_SIZE_FIRST, &sendPos, &sendAcquiredSize);
    memcpy(sendPos, strToSendFrist, SEND_SIZE_FIRST);
    BFX_QfifoSendCommit(&fifo);
    
    BFX_QfifoSendAcquireNoSplit(&fifo, SEND_SIZE_SECOND, &sendPos, &sendAcquiredSize);
    memcpy(sendPos, strToSendSecond, sizeof(strToSendSecond));
    BFX_QfifoSendCommit(&fifo);

    BFX_QfifoRecvAcquireNoSplit(&fifo, RECV_SIZE_FIRST, &recvPos, &recvAcquiredSize);
    ASSERT_STREQ(recvPos, strToSendFrist);
    BFX_QfifoRecvCommit(&fifo);
    
    BFX_QfifoRecvAcquireNoSplit(&fifo, RECV_SIZE_SECOND, &recvPos, &recvAcquiredSize);
...
TEST_F(EmFifoTestNoSplit, ProduceOverHead) {
    ...
    uint16_t freeSize = BFX_QfifoFreeNoSplitSize(&fifo);
    ASSERT_EQ(freeSize, 0);

    BFX_QfifoSendCommit(&fifo);
    freeSize = BFX_QfifoFreeNoSplitSize(&fifo);
    ASSERT_EQ(freeSize, 0);
...
TEST_F(EmFifoTestNoSplit, ConsumeOverHead) {
    ...
    uint16_t freeSize = BFX_QfifoRecvNoSplitSize(&fifo);
    ASSERT_EQ(freeSize, 0);

    BFX_QfifoRecvCommit(&fifo);
    freeSize = BFX_QfifoRecvNoSplitSize(&fifo);
    ASSERT_EQ(freeSize, 0);
}
...
TEST_F(EmFifoTestNoSplit, RefuseMultiProducer) {
    ...
    BFX_QfifoSendAcquireNoSplit(&fifo, SEND_SIZE, &sendPos1, &sendAcquiredSize1);
    BFX_QfifoSendAcquireNoSplit(&fifo, SEND_SIZE, &sendPos2, &sendAcquiredSize2);
...
TEST_F(EmFifoTestNoSplit, RefuseMultiConsumer) {
    ...
    BFX_QfifoRecvAcquireNoSplit(&fifo, RECV_SIZE, &recvPos1, &recvAcquiredSize1);
    BFX_QfifoRecvAcquireNoSplit(&fifo, RECV_SIZE, &recvPos2, &recvAcquiredSize2);
